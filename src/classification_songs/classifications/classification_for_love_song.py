...
from classification_songs.configorations.configoration import (info_queue, LOVE_COMMON, LOVE_LESS_COMMON, LOVE_MOST_COMMON)
import numpy as np
...
class ClassificationForLoveSong(ClassificationInterface):
    ...
    def calculate_final_score(self, song_info: SongInfo)->float:
        final_score = 0.35 * self.calculate_sound_score(song_info) + 0.50 * self.calculate_score_words(song_info) + 0.15 * self.calculate_score_length(song_info)
        return final_score
    ...
    def comparison_type(self)->tuple[bool, float]:
        try:
            ...
            love_song = (self.calculate_final_score(song_info) > 25, self.calculate_final_score(song_info))
            if not dc_with_types.love_song is None:
