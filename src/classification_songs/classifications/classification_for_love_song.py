...
class ClassificationForLoveSong(ClassificationInterface):
    ...
    @staticmethod
    def get_song_from_queue(timeout: int = 5)->Optional[Types]:
        if main_queue is None:
            logger.error('Redis connection is None')
            return None
        try:
            res = main_queue.brpop('love_song', timeout=timeout)
            if not res:
                logger.debug('No song found in "love_song" queue (timeout)')
                return None
            _q, info_json = res
            ...
            logger.error(f'Failed to fetch/parse from "love_song": {e}')
            return None
    ...
    def calculate_score_words(self, song_info: SongInfo)->float:
        ...
        logger.debug(f'Score words for song {song_info.song_name} : {final_score}')
        return final_score
    ...
    def calculate_score_length(self, song_info: SongInfo)->float:
        ...
        if 150 <= length < 280:
            ...
            score_length += 70.0
        logger.debug(f'Score length for song {song_info.song_name} : {score_length}')
        return score_length
    ...
    def drums_scor(self, song_info: SongInfo) -> float:
        ...
        drums = sound['drums']
        tempo = as_scalar(drums.get('tempo', 84) or 84.0)  # ← סקאלר
        ibi_std_val = as_scalar(drums.get('ibi_std', 0.5) or 0.5)  # ← סקאלר
        ibi_std = ibi_std_val if np.isfinite(ibi_std_val) else 0.5
        onset_density = as_scalar(drums.get('onset_density', 1.3) or 1.3)  # ← סקאלר
        ...
        logger.debug(f'drums score for song {song_info.song_name} : {drum_score}')
    ...
    def bass_scor(self, song_info: SongInfo) -> float:
        ...
        sound = self.get_sound_details(song_info)
        bass = sound['bass']
        low_ratio = as_scalar(bass.get('low_ratio', 0.18) or 0.18)  # ← סקאלר
        ...
        logger.debug(f'bass score for song {song_info.song_name} : {bass_score}')
    ...
    def others_scor(self, song_info: SongInfo) -> float:
        ...
        sound = self.get_sound_details(song_info)
        other = sound['other']
        centroid = as_scalar(other.get('centroid', 2300.0) or 2300.0)  # ← סקאלר
        dr_db = as_scalar(other.get('dr_db', 11.0) or 11.0)  # ← סקאלר
        bright_score = 1 - (min(abs(centroid - 2300) / 1100, 1.0) * 1.0)
        dr_scor = 1 - (min(abs(dr_db - 11) / 5, 1.0) * 1.0)
        other_score = as_scalar(70.0 * bright_score + 30.0 * dr_scor)  # ← סקאלר
        logger.debug(f'other score for song {song_info.song_name} : {other_score}')
        return other_score
    ...
    def calculate_sound_score(self, song_info: SongInfo) -> float:
        ...
        logger.debug(f'sound score for song {song_info.song_name} : {sound_score}')
        return sound_score
    ...
    def calculate_final_score(self, song_info: SongInfo)->float:
        ...
        final_score = (0.35 * as_scalar(self.calculate_sound_score(song_info))
                       + 0.50 * self.calculate_score_words(song_info)
                       + 0.15 * self.calculate_score_length(song_info))
        logger.debug(f'final score for song {song_info.song_name} : {final_score}')
        return final_score
    ...
    def comparison_type(self) -> None:
        ...
        try:
            ...
            dc_with_types.love_song = final_score
            main_queue.lpush(self.queue_name, dc_with_types.to_json())
            logger.debug(f'pushed update Types with love song genre to queue {self.queue_name}')
        except Exception as e:
            logger.error(f'love worker failed: {e}')
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            logger.error(f'love crash-loop guard: {e}')
            sleep(1)
